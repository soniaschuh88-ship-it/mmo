...
//!
//! The world coordinate system is **2D** (tile-based). Asset IRs must
//! produce data that the 2D tile renderer in `game.html` can consume
//! directly — no voxel z-stacks.
...
pub enum AssetIntent {
    /// A 2-D tile map (structure, base, dungeon layout).
    ///
    /// Replaced the old `VoxelStructure` (3-D) intent.  The world uses a
    /// flat tile renderer, so all spatial data is `(width, height)`.
    TileMap,
...
///
/// For Synthesis AI agents the `natural_language_spec` is generated by
/// NVIDIA NIM; for human designers it is typed text.
...
pub struct AssetBlueprint {
    ...
    ///
    /// Examples: `"no floating tiles"`, `"max_drop_rate <= 0.05"`.
...
pub enum CompiledAsset {
    /// 2-D tile map (structures, bases, dungeon layouts).
    TileMap(TileMapIR),
...
/// A deterministically generated 2-D tile structure.
///
/// Replaces the old `VoxelChunkIR` (3-D).  The coordinate system is flat:
/// `tiles[y * width + x]` → index into `tile_palette`.
///
/// Index `0` is always the "empty / passthrough" tile.
///
/// This is directly consumable by the `game.html` 2-D renderer:
/// `BM[ty][tx]` maps to one of these palette entries.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct TileMapIR {
    /// Stable string id (url-safe, derived from spec).
    pub id: String,

    /// Width and height in tiles.
    pub size: (u32, u32),

    /// Determinism seed (matches blueprint seed).
    pub seed: u64,

    /// Ordered list of tile type names.
    /// Index 0 = empty/air.  Renderer maps these to biome colors.
    pub tile_palette: Vec<String>,

    /// Flat tile data: `tiles[y * width + x]` = palette index.
    pub tiles: Vec<u32>,
}
